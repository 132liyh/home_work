<template>
    <view>
        ...
        <view class="end-cont" :class="{dis:btnnum === 0}">
            <view class="end-cont-list"
                  v-for="(item,key) in designData"
                  :key="key" hover-class="active-click"
                  :hover-stay-time="70"
                  @click="designClick(item)"
                  @longpress="designLong(item,key)">
                <image class="list-img" :src="item.imagePath" />
                <view class="list-box">
                    <view class="name">{{item.name}}-{{item.sex}}</view>
                    <text class="workYear">{{item.workYear}}年</text>
                </view>
            </view>
        </view>
        <view class="end-cont" :class="{dis:btnnum === 1}">
            <view class="uni-form-item">
                <input class="uni-input" placeholder="名字" v-model="name" />
                <input class="uni-input" placeholder="年龄" v-model="age"/>
                <view class="row">
          <textarea
                  class="text"
                  maxlength="5000"
          ...
          />
                ...
                <input class="uni-input" placeholder="工作年限" v-model="workyear"/>
...
<script>
    import {DeleteCase, DeleteDesigner, designerInfo, DesignerRegistration} from "../../utils/api";
    ...
    export default {
        ...
        methods: {
            ...
            designClick({id}){
                api.jump(viewName.designEdtail,{
                    id
                })
            },
            designLong({id}){
...
<style scoped lang="scss">
    ...
    .sex_wapper {
        ...
        align-items: center;
        padding:turn(30);
        border-bottom: turn(1) solid #efefef;
    }
