<template>
  <view>
    ...
    <view class="end-cont" :class="{dis:btnnum == 0}">设计师</view>
    <view class="end-cont" :class="{dis:btnnum == 1}">
      <view class="uni-form-item">
        <input class="uni-input" @blur="hanldeNmae" placeholder="名字" />
        <input class="uni-input" @blur="hanldeAge" placeholder="年龄" />
        ...
        <view class="sex_wapper">
          <view class="flex1 boy" :class="sexIndex == 0 ? 'active' :''" data-index="0" @click="checkSex">
            <view class="sex"></view>
            <view class="txt">男</view>
          </view>
          <view
            class="flex1 girls"
            :class="sexIndex == 1 ? 'active':''"
            data-index="1"
            @click="checkSex"
          >
            <view class="sex"></view>
        ...
        <input class="uni-input" @blur="hanldeworkyare" placeholder="工作年限" />
        <button type="primary" @click="hanldreGistered">注册</button>
      </view>
    </view>
  </view>
</template>
...
<script>
import { designerInfo, DesignerRegistration } from "../../utils/api";
...
export default {
  ...
  methods: {
      ...
      upload(file[key])
        .then(({ data }) => {
          if (type === "detail") {
            this.designerlImg = [
              ...this.designerlImg,
              `${baseUrl}file/getFile?filePath=${data}`
            ];
          } else {
    ...
    selImg(type) {
      uni.chooseImage({
        count: type === "detail" ? 5 - this.designerlImg.length : 1,
...
.uni-form-item > .case-input {
  background-color: #ffffff;
}

.uni-form-item > .sex_wapper {
  background-color: #ffffff;
}
